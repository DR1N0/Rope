...
class Models():
    def __init__(self): 
        ...
        self.logger.info(f"Face detection providers: {self.providers}")
    ...
    def run_swapper(self, image, embedding, output):
        try:
            if not self.swapper_model:
                ...
                # Pre-allocate numpy buffers for DirectML (avoids allocation overhead)
                if 'DmlExecutionProvider' in self.providers and self.device_type == 'cpu':
                    self._image_buffer = np.empty((1, 3, 128, 128), dtype=np.float32)
                    self._embedding_buffer = np.empty((1, 512), dtype=np.float32)
                    self.logger.info("✓ Pre-allocated DirectML buffers")
            ...
            if 'DmlExecutionProvider' in self.providers and self.device_type == 'cpu':
                ...
                np.copyto(self._image_buffer, image.cpu().numpy())
                np.copyto(self._embedding_buffer, embedding.cpu().numpy())
                
                # DirectML is not thread-safe, use global lock
                with directml_lock:
                    result = self.swapper_model.run(['output'], {'target': self._image_buffer, 'source': self._embedding_buffer})[0]
