...
class DeviceManager:
    ...
    def get_detection_providers(self) -> list:
        """
        Get ONNX Runtime providers for face detection models.
        On AMD ROCm, use CPU only to avoid crashes.
        On NVIDIA CUDA, use GPU.
        """
        if self.device_type == 'cuda':
            device_name = torch.cuda.get_device_name(0)
            if 'AMD' in device_name or 'Radeon' in device_name:
                # AMD ROCm: Use CPU for face detection to avoid crashes
                providers = ['CPUExecutionProvider']
                self.logger.info("Face detection providers (AMD workaround): ['CPUExecutionProvider']")
                return providers
            else:
                # NVIDIA CUDA: Use GPU for everything
    ...
    def get_detection_device_info(self) -> tuple:
        ...
        On AMD ROCm, returns CPU device to avoid crashes.
        """
        if self.device_type == 'cuda':
            device_name = torch.cuda.get_device_name(0)
            if 'AMD' in device_name or 'Radeon' in device_name:
                # AMD ROCm: Use CPU device for detection
                return ('cpu', 'cpu')
            else:
                # NVIDIA CUDA: Use GPU
